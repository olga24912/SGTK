 ...
 * writeFull < fileName > --- вывести весь граф в файлы в dot формате с префиксом файла < fileName >, автоматически файлы склеются в файл tmp.ps который будет находится в папке из которой программа запускалась. 
 * WriteLocal < fileName > < vertexID > < dist > --- вывести все вершины на расстояние не более чем < dist > от вершины < vertexID >. Способ вывода аналогичем writeFull. 
 * writeAllLocal < fileName > < dist > --- вывести для каждой вершины ее окрестность размера < dist > в файлы с префиксом < fileName >
 * writeLocalSeg < fileName > < vertexIDStart > < vertexIDFinish > < dist > --- вывести все окрестности вершин с id от  < vertexIDStart > до < vertexIDFinish >  
 * writeBig < prefixFileName > < size > --- вывести все компоненты, размер которых хотя бы size.  
 * writeAlongPath < prefixFileName > < libNum > < dist > < minRefPathSize > --- вывести все вершины вдоль пути из библиотеки < libNum >, на расстояние не более < dist > от пути. Длина пути, вдоль которого происходит вывод должна быть как минимум < minRefPathSize >
 * setFileVNotPathWithAllLib --- выводить только файлы, которые не являются простыми пытями, состояими из всех библиотек.
 * setFileVWithDifInLib < libNum >* --- выводить только файлы, в которых есть две вершины, между которыми есть хотя бы одно ребро из вышеперечисленных библиотек и хотя бы представителя одной библиотеки нет. 
 * setMaxVEinOneFile < maxVertNum > < maxEdgeNum > --- установить максимальное количество вершин в одном файле и максимальное количество ребер. 
