...
int Graph::getTargetLength(int id) {
    return targetLen[id];
}

void Graph::incTargetCover(int id, double x) {
    targetCoverage[id] += x;
}

int Graph::addVertex(int id, string name, double cov, int len) {
    int v = (int)graph.size();
    graph.push_back(vector<int>());
    targetId[name] = id;
    vById.resize(max(vById.size(), (unsigned int)id + 1), -1);
    vById[id] = v;
...
void Graph::incEdgeWeight(int vId, int uId) {
...
vector<int> Graph::getEdgesWeight(int v) {
    vector<int> w;
...
void Graph::delEdges(int v, int k) {
    graph[v].resize(graph[v].size() - k);
}

void Graph::sortEdgeByWeight(int v) {
...
void Graph::writeGraphDotFormat(string fileName) {
...
void Graph::filterByContigLen(int minContigLen) {
    Graph::minContigLen = minContigLen;
}

void Graph::filterByEdgeWeight(int minEdgeWeight) {
    for (int v = 0; v < (int)graph.size(); ++v) {
        for (int i = start[v]; i < (int)graph[v].size(); ++i) {
