...
void builder::graph_builder::RNASplitGraphBuilder::incEdgeWeight(seqan::BamAlignmentRecord read1,
    ...
    INFO("incEdgeWeight read1 " << read1.beginPos << " " << (read1.beginPos + seqan::getAlignmentLengthInRef(read1)));
    INFO("incEdgeWeight read1 " << read2.beginPos << " " << (read2.beginPos + seqan::getAlignmentLengthInRef(read2)));

    assert(seqan::isUniqueMapRead(read1));
    assert(seqan::isUniqueMapRead(read2));

    int target1 = get1Target(read1);
    std::pair<int, int> t1c = getCoord(read1);
    int target2 = get2Target(read2);
    std::pair<int, int> t2c = getCoord(read2);
    ...
    int e2 = changeEdges(pairTarget(target2),
                std::make_pair(graph->getTargetLen(target2) - t2c.first, graph->getTargetLen(target2) - t2c.second),
                pairTarget(target1),
                std::make_pair(graph->getTargetLen(target1) - t1c.first, graph->getTargetLen(target1) - t1c.second));
