...
namespace filter {
    namespace scaffolder {
        void
        ScaffoldStrategyLeaveConnection::addConnection(Scaffolds *scaffolds, Filter *graph, std::vector<int> minW) {
            DEBUG("addConnection");
            topSort(graph);
            findCycle(graph);
        ...
        void ScaffoldStrategyLeaveConnection::addConnectionForLeaves(Scaffolds *scaffolds, Filter *graph, int dir,
            ...
            for (int i = 0; i < n; ++i) {
                ...
                for (int e : edges) {
                    ...
                    if (u == -1 || u == to) {
                        u = to;
                        wu[graph->getLibType(graph->getEdgeLib(e))] = std::max(
                                wu[graph->getLibType(graph->getEdgeLib(e))],
                                graph->getEdgeWeight(e));
                    } else if (v == -1 || v == to) {
                        v = to;
                        wv[graph->getLibType(graph->getEdgeLib(e))] = std::max(
                                wv[graph->getLibType(graph->getEdgeLib(e))],
                                graph->getEdgeWeight(e));
                    } else {
                        u = -2;
                        break;
