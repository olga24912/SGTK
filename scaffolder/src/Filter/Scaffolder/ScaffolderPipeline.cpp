            ...
            for (int wp = 1; wp < 25; ++wp) {
                for (int ws50 = 1; ws50 < 25; ++ws50) {
                    ...
                    INFO("add scaffolds with minPairWeight=" << wp << " minSplit50Weight=" << ws50
                                                             << " minPairAcceptWeight="
                                                             << (int) (wp * 0.75 + ws50 * 2.25)
                                                             << " minSplit50AcceptWeight"
                                                             << (int) (wp * 0.75 + ws50 * 1.25));

                    scafol.addConnection(&scaffolds, graph, std::vector<int>(
                            {0, 0, (int) (wp * 0.75 + ws50 * 2.25), (int) (wp * 0.75 + ws50 * 1.25), 0, 0}));
                }
            }
        }
